...
namespace qc {
// A CoordIndex corresponds to node in the SLM grid, where an atom can be placed (or not).
using CoordIndex   = std::uint32_t;
using CoordIndices = std::vector<CoordIndex>;
// A HwQubit corresponds to an atom in the neutral atom architecture. It can be used as qubit or not and occupies a
// certain position in the architecture.
using HwQubit      = uint32_t;
using HwQubits     = std::set<HwQubit>;
using HwPositions  = std::vector<HwQubits>; // TODO make consistent use between set and vector
// A Qubit corresponds to a qubit in the quantum circuit. It can be mapped to a hardware qubit.
...
using SwapOrMove   = std::pair<HwQubit, HwQubit>; // TODO remove unclear naming
